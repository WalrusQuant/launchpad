...
- **Symbol outline** (`symbols.js`, Cmd+Shift+O): a fuzzy palette of the file's symbols. Opens instantly from the Lezer syntax tree (`extractSymbols(tree, docText)` — pure, Vitest-covered: JS funcs/classes/methods, Python defs/classes, Rust items, Markdown headings), then upgrades to the language server's `textDocument/documentSymbol` result when LSP is enabled (`normalizeLspSymbols` flattens both hierarchical `DocumentSymbol[]` and flat `SymbolInformation[]`, also pure + tested). Palette UI lives in `main.js`, reusing the quick-open row styles.
...
- **Language server (opt-in, `editorLanguageServer`, default off)**: `lsp.rs` is a process host modeled on `pty.rs` — it spawns external language servers (typescript-language-server for JS/TS, rust-analyzer for Rust, pyright for Python), a reader thread parses `Content-Length`-framed messages and emits them as `lsp-message` events, `lsp_send` frames outgoing messages to stdin. One server per `{language}:{project_path}`. The pure framing fns (`encode_lsp_message` / `read_lsp_message`) are unit-tested, and `#[ignore]`d e2e tests drive each real server through `initialize`/`didOpen` and assert diagnostics (skipped when the binary isn't installed, so CI stays green). Frontend `lspclient.js` runs `@codemirror/lsp-client` over a Tauri-IPC `Transport`, wired into the editor via an `lspCompartment`. Uses `languageServerExtensions()` (the full bundle: diagnostics, completion, hover, signature help, and go-to-def / rename / format / find-references keymaps). `shutdownAllLsp` stops servers on project teardown so the reload doesn't orphan them.
...
### Language Server (LSP)
- `lsp_start(project_path, language) -> server_id` — spawn (or reuse) a language server for the project+language; idempotent. Returns `"{language}:{project_path}"`. `language` ∈ {typescript, javascript, rust, python}. Errors if no server is configured or the binary isn't on PATH (frontend degrades silently).
